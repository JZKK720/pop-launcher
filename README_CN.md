...
![版本](https://img.shields.io/badge/版本-1.0.10-blue)
...
- 修复布局切换回归：从紧凑 Dock 恢复到工作区后，`+ 新增应用` 仍可能保持隐藏
- 现在切换到工作区时会强制重新渲染网格，确保新增与编辑流程恢复可见并可用
...
- 修复紧凑 Dock 回归问题：最小化任务栏模式下 `+ 新增应用` 仍可能出现并放大交互区域
- 紧凑模式下现在完全隐藏新增按钮；新增应用仍在工作区模式可用
...
2. 下载 `智方云cubecloud Setup 1.0.10.exe`
...
```bash
npm run build
# 输出路径：dist/智方云cubecloud Setup 1.0.10.exe
```
